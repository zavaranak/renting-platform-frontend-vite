...
import useCreateGuest from "@/hook/create-guest.hook";
...
const GuestManager = (params: GuestManagerProps) => {
  ...
  const { createGuest, loading, newId } = useCreateGuest();
  const validateGuest = () => {
    const errors: { [key: string]: string } = {};
    if (editingGuest) {
      if (!editingGuest.firstName) errors.firstName = "First Name is required";
    ...
    <div className="space-y-4">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start">
          <DropdownMenuItem onClick={() => setOpenGuestList(true)}>
